...
| `strategy-rules.md` | Prose your Claude session and `search_knowledge_base` can read - the numeric thresholds live in `config/agent.yaml`. |
| `signature.md` | The sign-off on outgoing email, and the EU AI Act Article 50 line (`docs/safety.md`) for any project email a person forwards to a guest. Read by `core.adapters.base.Email.signature()`. |
| `disclosure.md` | The one-line AI disclosure to copy into any guest-facing text a `marketing_action` brief becomes (a flyer, an offer). |
...
`make doctor`'s "AI disclosure" check FAILs until `signature.md` and
`disclosure.md` both exist AND have been edited (not left byte-identical to
their `.example.md`) - this is a go-live blocker, not optional polish.
